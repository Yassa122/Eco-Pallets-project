...
import { KafkaService } from 'src/kafka/kafka.service'; // Import KafkaService
...
export class WishlistService {
  ...
  async addProductToWishlist(
    userId: string,
    { productId }: AddProductWishlistDto,
  ): Promise<Wishlist> {
    ...
    await this.kafkaService.sendMessage('wishlist-add', { userId, productId });
  ...
  async removeProductFromWishlist(
    userId: string,
    { productId }: RemoveProductWishlistDto,
  ): Promise<Wishlist> {
    ...
    await this.kafkaService.sendMessage('wishlist-remove', {
      userId,
      productId,
    });
  ...
  async addItemToCartFromWishlist(
    userId: string,
    productId: string,
    ...
    console.log(
      'Wishlist Products Detailed:',
      JSON.stringify(wishlist.products, null, 2),
    );
    ...
    if (isProductInCart) {
      throw new ConflictException(
        `Product with ID ${productId} is already in the cart.`,
      );
    }
    ...
    // Emit Kafka event after adding item to cart from wishlist
    await this.kafkaService.sendMessage('cart-add', { userId, productId });
